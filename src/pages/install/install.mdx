...
export const description = 'Инструкция по установке Localzet Server через Composer. Требования: PHP 8.1+, расширения sockets, pcntl, posix. Быстрый старт для разработки асинхронных PHP приложений.'
export const keywords = 'localzet server установка, установить localzet server, localzet server composer, localzet server requirements, php async server setup, php websocket server install'

# Инструкция по установке

Localzet Server фактически представляет собой пакет PHP-кода. Если у вас уже установлена среда PHP, вам нужно просто загрузить исходный код Localzet Server или демонстрационные примеры для запуска. Установка выполняется через Composer - стандартный менеджер пакетов для PHP.
...
Если установка не удалась или PHP был установлен иным способом, необходимо собрать расширения из исходного кода. Для этого используйте инструмент `phpize` и следуйте стандартной процедуре компиляции PHP расширений.
...
Чтобы поддерживать большее количество одновременных соединений, необходимо установить расширение `event` и выполнить оптимизацию ядра Linux (настройка параметров `ulimit`, `net.core.somaxconn` и других системных параметров).
...
1. Для установки расширения `pcntl` используйте стандартную процедуру компиляции PHP расширений с помощью `phpize`.

2. Для установки расширения `event` используйте стандартную процедуру компиляции PHP расширений с помощью `phpize` (для среды разработки этот шаг можно опустить).
