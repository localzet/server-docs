...
export const description = 'Протоколы прикладного уровня в Localzet Server: HTTP, WebSocket, Text, Frame, TCP, UDP. Поддержка пользовательских протоколов. Примеры использования различных протоколов.'
export const keywords = 'localzet server протоколы, php websocket protocol, php http protocol, php tcp udp, php custom protocols, php application layer protocols, php transport protocols'

# Протоколы прикладного уровня

По традиции разработка на PHP основана на веб-технологиях и использует HTTP в качестве прикладного протокола. Это позволяет нам не думать о том, как он устроен сам протокол, а возложить это на Nginx, Apache или другой веб-сервер. Но когда проект требует другого протокола (например, WebSocket) - это становится проблемой. Localzet Server решает эту проблему, предоставляя встроенные протоколы и возможность создания пользовательских.
